...
public abstract class JSONUtils {
	...
	public static JsonConfig defaultConfig() {
		...
		jsonConfig.setJsonBeanProcessorMatcher(new JsonBeanProcessorMatcher() {
			...
			@Override public Object getMatch(Class target, Set matches) {
				Object match = PROCESSOR_MATCHER_SUPPORT.getMatch(target, matches);
				if (match == null)
					match = DEFAULT.getMatch(target, matches);
		...
		for (JsonConfigurator configurator : CONFIGURATORS)
			configurator.configure(jsonConfig);
	 ...
	 * @return JSON 형식의 문자열
	 */
	public static String toJSONString(Object target) { return toJSON(target).toString(); }
	 ...
	 * @return {@link JSON}
	 */
	public static JSON toJSON(Object target) { return toJSON(target, defaultConfig()); }
